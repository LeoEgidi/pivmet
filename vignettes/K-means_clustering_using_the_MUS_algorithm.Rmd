...
In such situations, we may need a more robust version of the classical K-means. The pivots may be used as initial seeds for a classical K-means algorithm [@kmeans]. The function `piv_KMeans` works as the classical `kmeans` function, with some optional arguments
...
- `num.seeds`: If `alg.type` is `"KMeans"`, the number of different starting random seeds to be passed to `KMeans()`. Default is 10.
