...
- `piv.criterion`: one among the four different pivotal criteria described above and listed in @egidi2018relabelling: `MUS`, `maxsumint`, `minsumnoint`, `maxsumdiff`. `MUS` is the default criterion when `centers` $\le$ 4, whereas `maxsumdiff` is the default method when `centers` > 4.

- `iter.mus`: the number of different partitions used for building the co-association matrix $C$. Default is $10^3$.

- `prec.par`: with this argument the user may increase the powerful of the underlying MUS algorithm (see @egidi2018mus for details). The usual choice is $\min\{ 10, \underset{j}{\min}n_j \}$, where $n_j$ is the number of units belonging to the group $j, \ j=1,\ldots,k$.

- `alg.type`: the clustering technique applied to the raw data such that the partition is given in input to the `MUS` algorithm. One method among `KMeans` and `hclust` may be selected.  A robust choice is required here---by default, `KMeans` is used.
