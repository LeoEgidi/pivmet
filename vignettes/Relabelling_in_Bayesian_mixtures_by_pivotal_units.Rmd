...
There are two main functions designed for our procedure. The function `piv_MCMC`: 
  ...
  + **build a co-association matrix for the $n$ units**.  After $H$ MCMC iterations, the function implements a clustering procedure on $k$ groups, and through the optional argument `clustering` the user may choose among agglomerative or divisive hierarchical clustering. Using the latent formulation for mixture models, we denote with $[Z_i]_h$ the group allocation of the $i$-th unit at the $h$-th iteration. In such a way, a co-association matrix $C$ for the $n$ statistical units is built, where the single cell $c_{ip}$ is the fraction of times the unit $i$ and the unit $p$ belong to the same group along the $H$ MCMC iterations:
...
where $|\cdot|$ denotes the event indicator and $n_{ip}$ is the number of times the units $i, \ p$ belong to the same group along the sampling;
  ...
  + **find the pivots**. Using this matrix, the internal function `piv_sel` extracts some units, one for each group, which
...
Alternatively, when $k <5$, the user has the optional pivotal criterion `MUS` [@egidi2018mus] based on a sequential search within the matrix $C$.
  ...
  + **performs the relabelling algorithm** using the $k$ pivotal units as groups identifiers. The pivotal units previously detected play a central role, yielding to the following relabelling:
...
It yields the relabelled chains and the corresponding posterior estimates. The only argument `mcmc` takes as input the MCMC output from `piv_MCMC`.
...
We fit our model by simulating $H=15000$ samples from the posterior distribution of $(\boldsymbol{z}, \boldsymbol{\mu}, \boldsymbol{\phi}, \boldsymbol{\pi})$. If the argument ```software="rjags"``` is selected (the default option),  Gibbs sampling is performed by the package ```bayesmix```, and  the priors are:
...
with default values: $B_0=0.1$, $\nu_0 =20$, $g_0 = 10^{-17}$, $G_0 = 10^{16}$, $\boldsymbol{\alpha}=(1,1,\ldots,1)$. The users may specify their own hyperparameters with the ```priors``` arguments, declaring a names list such as: ```priors = list(mu_0=2, alpha = rep(2, k))```.
...
$\boldsymbol{\mu}_0, S_2, S_3$ and $\boldsymbol{\alpha}$ via \code{priors} argument in such a way: `priors =list(mu_0 = c(1,1), S2 = ..., S3 = ..., alpha = ...)}`,
...
If `software="rstan"`, the priors are specified as:
...
Once we obtain posterior estimates, label switching is likely to occurr. For such a reason, we need to relabel our chains as explained above. The function `piv_rel` performs our procedure, and for such task it only needs the `mcmc = res` argument. Once we correctly relabel the chains, we may plot the relabelled outputs through the function `piv_plot`, with different options for the argument `type`:
