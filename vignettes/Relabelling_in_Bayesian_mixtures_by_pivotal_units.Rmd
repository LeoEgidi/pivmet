...
The function ```piv_MCMC``` requires only three mandatory arguments: data ```y```, number of components ```k``` and number of MCMC iterations ```nMC```. It performs Gibbs sampling using the  ```runjags``` package for bivariate data by default. If `software="rstan"`, it performs Hamiltonian Monte Carlo (HMC) sampling. If `software="rjags"`, the priors are specified as:
