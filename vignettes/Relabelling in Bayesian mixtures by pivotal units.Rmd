...
The function `piv_rel` performs the procedure above, and for such task it only needs the `mcmc = rel` argument and `nMC`. Once we correctly relabel the chains, we may plot the relabelled outputs through the the function `piv_plot`, with different options for the argument `type`:
